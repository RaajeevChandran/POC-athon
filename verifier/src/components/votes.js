...
export default function Votes(props) {
    ...
    const {username,aadhar_image,init_ip,vote} = props.votesData
    const {signedVC,} = props.votesData
    // console.log("unverified vc is "+signedVC)
    const {holder,credentialSubject} = signedVC
    const {id} = holder
    ...
    <div className={classes.root}>
      <Accordion defaultExpanded>
        ...
        <AccordionDetails className={classes.details}>
          <div className={classes.column} >
          <Typography className={classes.heading}>{`IP ADDRESS   : `}<span>{init_ip}</span></Typography>
          <Typography className={classes.heading}>{`ID   : ${id}`}</Typography>
          <Typography className={classes.heading}>{`VOTED TO  : ${vote}`}</Typography>
